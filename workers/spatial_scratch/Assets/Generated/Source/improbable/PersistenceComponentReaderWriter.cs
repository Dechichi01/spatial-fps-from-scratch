...
namespace Improbable
{
    ...
    public class PersistenceReaderSubscriptionManager : SubscriptionManager<PersistenceReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<PersistenceReader>>> entityIdToReaderSubscriptions;
        ...
        public PersistenceReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(Persistence.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new PersistenceReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(Persistence.ComponentId, entityId =>
            {
        ...
        public override Subscription<PersistenceReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PersistenceReader>>>();
            }

            var subscription = new Subscription<PersistenceReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PersistenceReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<Persistence.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PersistenceReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<PersistenceReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class PersistenceWriterSubscriptionManager : SubscriptionManager<PersistenceWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<PersistenceWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public PersistenceWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(Persistence.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new PersistenceWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<PersistenceWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PersistenceWriter>>>();
            }

            var subscription = new Subscription<PersistenceWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PersistenceWriter>>();
                entityIdToWriterSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && componentUpdateSystem.HasComponent(Persistence.ComponentId, entityId)
                && componentUpdateSystem.GetAuthority(entityId, Persistence.ComponentId) != Authority.NotAuthoritative)
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PersistenceWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<PersistenceWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class PersistenceReader
    {
        ...
        public Persistence.Component Data
        {
            get
            {
                if (!IsValid)
                ...
                return EntityManager.GetComponentData<Persistence.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, Persistence.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, Persistence.ComponentId, value);
        ...
        private Dictionary<Action<Persistence.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<Persistence.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<Persistence.Update>, ulong>();
                }
        ...
        internal PersistenceReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
    ...
    public class PersistenceWriter : PersistenceReader
    {
        ...
        {
            ComponentUpdateSystem.AcknowledgeAuthorityLoss(EntityId, Persistence.ComponentId);
        }
    }
}
