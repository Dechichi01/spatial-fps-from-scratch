...
namespace Improbable
{
    ...
    public class PositionReaderSubscriptionManager : SubscriptionManager<PositionReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<PositionReader>>> entityIdToReaderSubscriptions;
        ...
        public PositionReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(Position.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new PositionReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(Position.ComponentId, entityId =>
            {
        ...
        public override Subscription<PositionReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PositionReader>>>();
            }

            var subscription = new Subscription<PositionReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PositionReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<Position.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PositionReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<PositionReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class PositionWriterSubscriptionManager : SubscriptionManager<PositionWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<PositionWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public PositionWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(Position.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new PositionWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<PositionWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PositionWriter>>>();
            }

            var subscription = new Subscription<PositionWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PositionWriter>>();
                entityIdToWriterSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && componentUpdateSystem.HasComponent(Position.ComponentId, entityId)
                && componentUpdateSystem.GetAuthority(entityId, Position.ComponentId) != Authority.NotAuthoritative)
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PositionWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<PositionWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class PositionReader
    {
        ...
        public Position.Component Data
        {
            get
            {
                if (!IsValid)
                ...
                return EntityManager.GetComponentData<Position.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, Position.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, Position.ComponentId, value);
        ...
        private Dictionary<Action<Position.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<Position.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<Position.Update>, ulong>();
                }
        ...
        internal PositionReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
    ...
    public class PositionWriter : PositionReader
    {
        ...
        {
            ComponentUpdateSystem.AcknowledgeAuthorityLoss(EntityId, Position.ComponentId);
        }
    }
}
