...
namespace Improbable.Gdk.Health
{
    ...
    {
        public class HealthRegenComponentViewStorage : IViewStorage, IViewComponentStorage<Snapshot>,
            IViewComponentUpdater<Update>
        {
            ...
            public void ApplyUpdate(long entityId, in Update update)
            {
                ...
                {
                    data.DamagedRecently = update.DamagedRecently.Value;
                }

                if (update.RegenCooldownTimer.HasValue)
                {
                    data.RegenCooldownTimer = update.RegenCooldownTimer.Value;
                }

                if (update.CooldownSyncInterval.HasValue)
                {
                    data.CooldownSyncInterval = update.CooldownSyncInterval.Value;
                }

                if (update.RegenPauseTime.HasValue)
                {
                    data.RegenPauseTime = update.RegenPauseTime.Value;
                }

                if (update.RegenInterval.HasValue)
                {
                    data.RegenInterval = update.RegenInterval.Value;
                }

                if (update.RegenAmount.HasValue)
                {
                    data.RegenAmount = update.RegenAmount.Value;
                }
