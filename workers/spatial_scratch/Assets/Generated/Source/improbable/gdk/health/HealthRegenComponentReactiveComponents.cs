    ...
    public partial class HealthRegenComponent
    {
        public class UpdateReactiveComponentManager : IReactiveComponentManager
        {
            public void PopulateReactiveComponents(ComponentUpdateSystem updateSystem, EntityManager entityManager, WorkerSystem workerSystem, World world)
            {
                ...
                for (int i = 0; i < updatesReceived.Count; ++i)
                {
                    ...
                    List<Update> updates;
                    if (entityManager.HasComponent<Improbable.Gdk.Health.HealthRegenComponent.ReceivedUpdates>(entity))
                    {
                        updates = entityManager.GetComponentData<Improbable.Gdk.Health.HealthRegenComponent.ReceivedUpdates>(entity).Updates;
                    }
                    else
                    {
            ...
            {
                HealthRegenComponent.ReferenceTypeProviders.UpdatesProvider.CleanDataInWorld(world);
            }
        }
        ...
        public class LegacyAuthorityComponentManager : IReactiveComponentManager
        {
            public void PopulateReactiveComponents(ComponentUpdateSystem updateSystem, EntityManager entityManager, WorkerSystem workerSystem, World world)
            {
                ...
                foreach (var entityId in world.GetExistingManager<EntitySystem>().GetEntitiesAdded())
                {
                    ...
                    entityManager.AddComponent(entity,
                        ComponentType.Create<NotAuthoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>());
                }
            ...
            private void ApplyAuthorityChange(Unity.Entities.Entity entity, Authority authority, global::Improbable.Gdk.Core.EntityId entityId, World world, EntityManager entityManager)
            {
                switch (authority)
                {
                    case Authority.Authoritative:
                        if (!entityManager.HasComponent<NotAuthoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>(entity))
                        {
                            LogInvalidAuthorityTransition(Authority.Authoritative, Authority.NotAuthoritative, entityId);
                            return;
                        }

                        entityManager.RemoveComponent<NotAuthoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>(entity);
                        entityManager.AddComponent(entity, ComponentType.Create<Authoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>());

                        break;
                    case Authority.AuthorityLossImminent:
                        if (!entityManager.HasComponent<Authoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>(entity))
                        {
                            LogInvalidAuthorityTransition(Authority.AuthorityLossImminent, Authority.Authoritative, entityId);
                            return;
                        }

                        entityManager.AddComponent(entity, ComponentType.Create<AuthorityLossImminent<Improbable.Gdk.Health.HealthRegenComponent.Component>>());
                        break;
                    case Authority.NotAuthoritative:
                        ...
                        if (entityManager.HasComponent<AuthorityLossImminent<Improbable.Gdk.Health.HealthRegenComponent.Component>>(entity))
                        {
                            entityManager.RemoveComponent<AuthorityLossImminent<Improbable.Gdk.Health.HealthRegenComponent.Component>>(entity);
                        }

                        entityManager.RemoveComponent<Authoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>(entity);
                        entityManager.AddComponent(entity, ComponentType.Create<NotAuthoritative<Improbable.Gdk.Health.HealthRegenComponent.Component>>());
                        break;
                }
            }
            ...
            private void LogInvalidAuthorityTransition(Authority newAuthority, Authority expectedOldAuthority, global::Improbable.Gdk.Core.EntityId entityId)
            {
                ...
                //     .WithField("Component", "Improbable.Gdk.Health.HealthRegenComponent")
                // );
            }
        }
    }
