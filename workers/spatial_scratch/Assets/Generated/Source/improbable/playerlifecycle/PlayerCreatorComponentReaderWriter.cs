...
namespace Improbable.PlayerLifecycle
{
    ...
    public class PlayerCreatorReaderSubscriptionManager : SubscriptionManager<PlayerCreatorReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<PlayerCreatorReader>>> entityIdToReaderSubscriptions;
        ...
        public PlayerCreatorReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(PlayerCreator.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new PlayerCreatorReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(PlayerCreator.ComponentId, entityId =>
            {
        ...
        public override Subscription<PlayerCreatorReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PlayerCreatorReader>>>();
            }

            var subscription = new Subscription<PlayerCreatorReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PlayerCreatorReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<PlayerCreator.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PlayerCreatorReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<PlayerCreatorReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class PlayerCreatorWriterSubscriptionManager : SubscriptionManager<PlayerCreatorWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<PlayerCreatorWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public PlayerCreatorWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(PlayerCreator.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new PlayerCreatorWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<PlayerCreatorWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PlayerCreatorWriter>>>();
            }

            var subscription = new Subscription<PlayerCreatorWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PlayerCreatorWriter>>();
            ...
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PlayerCreatorWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<PlayerCreatorWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class PlayerCreatorReader
    {
        public bool IsValid;
        ...
        public PlayerCreator.Component Data
        {
            get
            {
                if (!IsValid)
                ...
                return EntityManager.GetComponentData<PlayerCreator.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, PlayerCreator.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, PlayerCreator.ComponentId, value);
        ...
        private Dictionary<Action<PlayerCreator.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<PlayerCreator.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<PlayerCreator.Update>, ulong>();
                }
        ...
        internal PlayerCreatorReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
    ...
    public class PlayerCreatorWriter : PlayerCreatorReader
    {
        ...
        {
            ComponentUpdateSystem.AcknowledgeAuthorityLoss(EntityId, PlayerCreator.ComponentId);
        }
    }
}
