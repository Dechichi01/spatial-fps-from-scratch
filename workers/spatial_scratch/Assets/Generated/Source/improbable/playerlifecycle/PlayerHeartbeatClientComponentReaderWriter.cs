...
namespace Improbable.PlayerLifecycle
{
    ...
    public class PlayerHeartbeatClientReaderSubscriptionManager : SubscriptionManager<PlayerHeartbeatClientReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<PlayerHeartbeatClientReader>>> entityIdToReaderSubscriptions;
        ...
        public PlayerHeartbeatClientReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(PlayerHeartbeatClient.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new PlayerHeartbeatClientReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(PlayerHeartbeatClient.ComponentId, entityId =>
            {
        ...
        public override Subscription<PlayerHeartbeatClientReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PlayerHeartbeatClientReader>>>();
            }

            var subscription = new Subscription<PlayerHeartbeatClientReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PlayerHeartbeatClientReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<PlayerHeartbeatClient.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PlayerHeartbeatClientReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<PlayerHeartbeatClientReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class PlayerHeartbeatClientWriterSubscriptionManager : SubscriptionManager<PlayerHeartbeatClientWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<PlayerHeartbeatClientWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public PlayerHeartbeatClientWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(PlayerHeartbeatClient.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new PlayerHeartbeatClientWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<PlayerHeartbeatClientWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PlayerHeartbeatClientWriter>>>();
            }

            var subscription = new Subscription<PlayerHeartbeatClientWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PlayerHeartbeatClientWriter>>();
            ...
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PlayerHeartbeatClientWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<PlayerHeartbeatClientWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class PlayerHeartbeatClientReader
    {
        public bool IsValid;
        ...
        public PlayerHeartbeatClient.Component Data
        {
            get
            {
                if (!IsValid)
                ...
                return EntityManager.GetComponentData<PlayerHeartbeatClient.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, PlayerHeartbeatClient.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, PlayerHeartbeatClient.ComponentId, value);
        ...
        private Dictionary<Action<PlayerHeartbeatClient.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<PlayerHeartbeatClient.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<PlayerHeartbeatClient.Update>, ulong>();
                }
    ...
    public class PlayerHeartbeatClientWriter : PlayerHeartbeatClientReader
    {
        ...
        {
            ComponentUpdateSystem.AcknowledgeAuthorityLoss(EntityId, PlayerHeartbeatClient.ComponentId);
        }
    }
}
